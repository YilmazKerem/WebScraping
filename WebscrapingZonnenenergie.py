...
def SendFirebase(Json,Datum): #Json is een list
    ...
    Resultaat = Firebase.get(f"/Zonnenenergie/{Datum}/",None)
    if Resultaat == None: 
        for Data in Json :
            result = Firebase.post(f"/Zonnenenergie/{Datum}/",Data) 
        ...
        for Data in Json:
            for Value in Resultaat:
                ...
                if(ValueList['RealTimeAvailable']== False and Data['RealTimeAvailable'] == True and 
                    ...
                    result1 = Firebase.put(f"/Zonnenenergie/{Datum}/{Value}", 'RealTimeAvailable', Data['RealTimeAvailable'])
                    result = Firebase.put(f"/Zonnenenergie/{Datum}/{Value}/RealTime", 'RealForcast', Data['RealTime']['RealForcast'])
                    
                    break


def ReadPageUrl(TodayDate):
    Scrape_Url = f"https://griddata.elia.be/eliabecontrols.prod/interface/solareforecasting/chartdataforzone?dateFrom={TodayDate}&dateTo={TodayDate}&sourceID=1"
